...
#     if isinstance(scale, (float, int)):
#         if scale <= 0:
#             raise ValueError('Invalid scale {scale}, must be positive.')
#         scale_factor = scale
...
class DepthImage:
    def __init__(self, file):
        ...
        if isinstance(file, np.ndarray):
            ...
            self.img = scio.loadmat(file)['A']      # (480, 640)  float max=0.7
    ...
    def rescale_2d(self, scale, interpolation='bilinear'):
        ...
        scale: 缩放尺度
        """
        self.img = imresize(self.img, scale, interp="bilinear")
        # self.img = imrescale(self.img, scale, interpolation=interpolation)
    ...
    #     return scale
    # def rotate(self, rota, border_value=0.7):
    #     """
    #     顺时针旋转 rota (角度)
    #     """
    ...
    def fuzz(self):
        ...
        连续两次resize
        """
        self.img = imresize(self.img, 0.25, interp='bilinear')
        self.img = imresize(self.img, 4.0, interp='bilinear')
...
class Image:
    def __init__(self, depthimg=None, rgbimg=None):
